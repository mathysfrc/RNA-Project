...
class MulticlassPerceptron:
    def __init__(self, num_classes=3, eta=0.05, max_iter=3000, tol=0.01):
...
def plot_decision_boundary(model, X, y):
    ...
    plt.xlabel('x1')
    plt.ylabel('x2')
    plt.title('Frontière de décision pour classification à 3 classes')
    plt.grid(True)
...
def load_data(filename):
    ...
    y = df.iloc[:, -1].values  # Étiquettes (supposées être 0, 1, 2 pour 3 classes)
    return X, y


# Exemple d'utilisation avec des données synthétiques pour 3 classes
def generate_synthetic_data(n_samples=100, n_features=2, n_classes=3):
...
X, y = generate_synthetic_data(n_samples=100, n_features=2, n_classes=3)

# Apprentissage
model = MulticlassPerceptron(num_classes=3, eta=0.03, max_iter=4000, tol=0.001)
